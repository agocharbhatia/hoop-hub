...
- Local eval runs every prompt variant at least once and currently covers 22 deterministic prompt-level runs; the custom-shot suite adds seven deterministic contract cases.
...
- Current required eval coverage includes pull-up mid-range grounding and shot-chart reconciliation, latest-ten trend chronology, top-five truncation hygiene, conditional records, win/loss splits, typed semantic lookup/split grounding, tracking-derived named-player matchup stats, stable made-three clip intent/FG3M mapping, named-defender clip fallback rejection, endpoint failure behavior, and global product-response diagnostic hygiene.
